...
export function Navbar() {
  ...
  return (
    <nav className="flex items-center justify-between h-16 px-6 border-b border-border-DEFAULT bg-bg-primary">
      <a href="/" className="text-[18px] font-bold text-accent-blue">CASUYA</a>
      ...
      <div className="flex items-center gap-3">
        ...
        {user ? (
          ...
          <Button variant="primary" className="!h-11" onClick={() => router.push('/auth')}>{t('nav.login', lang)}</Button>
        )}
        <button className="md:hidden text-[24px] text-text-primary" onClick={() => setMenuOpen(!menuOpen)}>☰</button>
      </div>
        ...
        <div className="fixed top-0 right-0 h-full w-72 bg-bg-secondary border-l border-border-strong z-50 p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            {user ? (
              ...
              <Button variant="primary" className="!h-11 w-full" onClick={() => router.push('/auth')}>{t('nav.login', lang)}</Button>
            )}
          </div>
        </div>
      )}
