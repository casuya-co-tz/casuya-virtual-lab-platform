...
export const FeatureComparison: React.FC<FeatureComparisonProps> = ({ lang, plans, features }) => {
  return (
    <div className="w-full overflow-x-auto rounded-xl border border-border-default bg-bg-secondary shadow-sm mt-12">
          ...
          {features.map((f) => (
            <tr key={f.key} className="hover:bg-bg-tertiary transition-colors">
              ...
              {plans.map((p) => {
                ...
                return (
                  <td key={p.slug} className="p-4 text-center text-text-muted">
                    {typeof spec === 'boolean' ? (
                      spec ? (
                      ...
                      <span className="font-semibold text-xs bg-bg-tertiary px-2 py-1 rounded border border-border-default">{spec}</span>
                    )}
                  </td>
                )
              })}
