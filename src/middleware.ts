...
function addCspHeaders(response: NextResponse): NextResponse {
  ...
  const csp = isDev
    ? `default-src 'self'; script-src 'self' 'nonce-${nonce}' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://*.supabase.co ws: wss:; font-src 'self' data:; frame-src 'self'; frame-ancestors 'self'; form-action 'self'; object-src 'none'`
    : `default-src 'self'; script-src 'self' 'nonce-${nonce}'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://*.supabase.co; font-src 'self' data:; frame-src 'self'; frame-ancestors 'self'; form-action 'self'; object-src 'none'`
  ...
  try {
    ...
    const supabase = mod.createServerClient(
      ...
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
      {
        cookies: {
          getAll() {
            return mod.parseCookieHeader(req.headers.get('cookie') ?? '').map((c: { name: string; value?: string }) => ({ name: c.name, value: c.value ?? '' }))
          },
          setAll() {},
        },
      }
    ...
    return { id: session.user.id, role: profile?.role || 'student' }
  } catch {
    return null
  }
}
